...
export const INJECT_MARK = 'data-dsh-lan-polyfill';

/** 免鉴权的自描述端点，方便客户端做自动发现。 */
export const INFO_PATH = '/__dsh_lan__/info';
 ...
 */
export function buildInfoPayload({ port, upstreamPort }, addresses = lanAddresses()) {
...
export function createLanProxy({
  ...
  const server = createServer((req, res) => {
    // 注意：/__dsh_lan__/info 不在这里处理，直接转发给上游 ——
      ...
      (upRes) => {
        ...
        upRes.on('end', () => {
          try {
            ...
            warn(`dsh-lan: HTML 注入失败，按原样返回 —— ${err?.message ?? err}`);
            res.writeHead(upRes.statusCode ?? 502, headers);
            res.end(Buffer.concat(chunks));
          }
        });
    ...
    proxyReq.on('error', (err) => {
      warn(`dsh-lan: 转发失败 ${req.method} ${req.url} —— ${err?.message ?? err}`);
      if (!res.headersSent) {
        res.writeHead(502, { 'content-type': 'text/plain; charset=utf-8' });
      }
      res.end('dsh-lan: upstream unreachable');
    });
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    proxyReq.on('error', (err) => {
      warn(`dsh-lan: WebSocket 转发失败 ${req.url} —— ${err?.message ?? err}`);
      try { socket.destroy(); } catch { /* 已关 */ }
    });
