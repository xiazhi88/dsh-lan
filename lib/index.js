...
// 这两件事原本要用户另外装一个包才有；内联进来是为了「只装一个包」。
import { installResponseCompression } from './vendor/compress.js';
...
export function apply(ctx, config = {}) {
  ...
  /**
   * 大响应压缩（内联自 dsh-web-mobile）。
   *
   * 长会话的 `session.history` 在手机上动辄几 MB，压一下差别很明显。
   * 它会 patch `http.ServerResponse.prototype`（进程级，卸载时还原）——
   * 这正是**不能再被别人挂第二次**的原因。
   */
  ctx.effect(() => installResponseCompression(), 'dshgo: 响应压缩');
